...
fn main() {
  App::new()
    ...
    .add_systems(Startup, setup)
    // Run our culling & buffer update system every frame
    .add_systems(Update, update_instance_buffers)
    .run();
}

fn setup(mut commands: Commands, mut meshes: ResMut<Assets<Mesh>>) {
  // Create a large number of instance data.
  let instances: Vec<InstanceData> = (1..=10)
  ...
  commands.spawn((
    ...
    // Do not insert an InstanceBuffer here; it will be created in update_instance_buffers.
    NoFrustumCulling, // Disable built-in frustum culling.
  ));
...
/// A GPU buffer containing per‑instance data.
#[derive(Component)]
...
// Implement ExtractComponent for InstanceBuffer so that it gets sent to the render world.
impl ExtractComponent for InstanceBuffer {
  type QueryData = &'static InstanceBuffer;
  type QueryFilter = ();
  // Here we assume Buffer is cheaply clonable as a handle.
  type Out = InstanceBuffer;

  fn extract_component(
    item: QueryItem<'_, Self::QueryData>,
  ) -> Option<Self::Out> {
    Some(InstanceBuffer { buffer: item.buffer.clone(), length: item.length })
  }
}

/// New system to perform basic per‑instance culling and update the instance buffer.
fn update_instance_buffers(
  mut commands: Commands,
  instance_query: Query<(Entity, &InstanceMaterialData)>,
  // Query the active camera transform; here we assume only one camera with the PanOrbitCamera marker.
  camera_query: Query<&GlobalTransform, With<PanOrbitCamera>>,
  render_device: Res<RenderDevice>,
) {
  // Use a simple distance threshold for culling. In a real app, use frustum culling.
  let cull_distance = 500.0;
  ...
  for (entity, instance_data) in &instance_query {
    let visible_instances: Vec<InstanceData> = instance_data
      .iter()
      .filter(|inst| {
        inst.position.distance(camera_transform.translation()) < cull_distance
      })
      .copied()
      .collect();

    // Update (or recreate) the GPU buffer only with visible instance data.
    let buffer = if !visible_instances.is_empty() {
      ...
      // If no instances are visible, create an empty buffer.
      render_device.create_buffer_with_data(&BufferInitDescriptor {
        label: Some("empty instance data buffer"),
        contents: &[],
        usage: BufferUsages::VERTEX | BufferUsages::COPY_DST,
      })
    };
    ...
    commands
      .entity(entity)
      .insert(InstanceBuffer { buffer, length: visible_instances.len() });
  }
}
...
impl Plugin for CustomMaterialPlugin {
  fn build(&self, app: &mut App) {
    ...
    // Add InstanceBuffer extraction so render systems can access it.
    app.add_plugins(ExtractComponentPlugin::<InstanceBuffer>::default());
    app
      ...
      .add_systems(
        ...
        (
          ...
          // update_instance_buffers now runs in the main app.
        ),
      );
  }
    ...
    descriptor.vertex.buffers.push(VertexBufferLayout {
      ...
      attributes: vec![
        VertexAttribute {
          ...
          offset: 0,
          shader_location: 3, // shader locations 0-2 are taken by position, normal, uv.
        },
        VertexAttribute {
          format: VertexFormat::Float32x4,
